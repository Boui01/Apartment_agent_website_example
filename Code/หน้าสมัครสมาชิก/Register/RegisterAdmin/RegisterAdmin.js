...
function RegisterAdmin({handleClose , setWaiting , setLogin , setLanguage}) {
    ...
    const handleSubmit = async (event) => {
        ...
        if(event.target.password.value !== event.target.password_confirmation.value){
            ...
            try {
                const response = await axios.post(`http://127.0.0.1:8000/api/register/admin/${setLogin.id}`, formData,{ 
                    headers: {
